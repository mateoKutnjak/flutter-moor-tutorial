...
part 'moor_database.g.dart'; // ? Does not exist in time of writing this line
...
@UseDao(tables: [
  Tasks,
  Tags // ? Needed to perform join queries
])
class TaskDao extends DatabaseAccessor<AppDatabase> with _$TaskDaoMixin {
  ...
  //   return (select(tasks)
  //         ..orderBy([
  //           (t) => OrderingTerm(expression: t.dueData, mode: OrderingMode.desc),
  ...
  Future insertTask(Insertable<Task> task) => into(tasks).insert(
      task); // ? Return value Future<int> returns id value (if it is auto increment column)
